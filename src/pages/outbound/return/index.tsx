...
type OutboundDetailsViews = Pick<
  OutboundDetail,
  "productName" | "lotNumber" | "quantity"
>;
...
const ReturnOutboundPage = () => {
  ...
  const outboundDetailsReturnedColumns = ({
    onRemove,
    onQuantityChange,
  }: OutboundDetailsProps): TableProps<OutboundDetailsViews>["columns"] => [
    {
      title: "Tên Sản Phẩm",
      dataIndex: "productName",
      key: "productName",
    ...
    },
    {
      ...
      key: "action",
      render: (_: unknown, record: OutboundDetailsViews, index: number) => (
        <Button
          type="link"
          danger
          icon={<DeleteOutlined />}
  ...
  const handleOnSubmit = () => {
    const data = selectedOutboundDetailsReturnData.map<OutboundReturnRequest>(
      (item) => ({
